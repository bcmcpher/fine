function se = feComputeEvidence_new(rmse1, rmse2, nbins, nmc, nboots, thr)
...
% pull the size of each distribution
...
%         ncos(boot, mc) = pdist2(nrmse1, nrmse2, 'cosine');
        ...
        % Earth Mover's Distance; cased to failover to matlab fxn if MEX fxn fails
        %try
...
% use thresholding and estimated p-value to 0 connection
if se.s.pval <= thr
    se.s.tmean = se.s.mean;
else
    se.s.tmean = 0;
end
