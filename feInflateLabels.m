function [ fh, ilabs, dbout ] = feInflateLabels(labels, mask, infl, nlog, outfile)
%% 
...
display('Loading labeled nifti...');

% read in aligned aparc+aseg .nii (read labels)
olabs = niftiRead(labels);
    ...
    for ii = 1:size(olabs.data, 1)
        for jj = 1:size(olabs.data, 2)
            for kk = 1:size(olabs.data, 3)
                ...
                if out(ii, jj, kk) == 0 && mask.data(ii, jj, kk) == 1
...
subplot(3, 2, 3); imagesc((mask.data(:,:,78) > 0) + (x > 0)); 
title('Original Labels + WM'); colorbar;

subplot(3, 2, 4); imagesc(mask.data(:,:,78) > 0); 
