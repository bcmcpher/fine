function [ pconn, rois, omat, olab, files ] = fnBuildDKNetworks(subj, track, lmax, rep, nclust, cacheDir)
...
if strcmp(track, {'tensor_'})
    ...
    files.out     = ['fn_' subj '_aparc+aseg_' track '_rep' rep '.mat'];
    files.csv     = ['fn_' subj '_aparc+aseg_' track '_rep' rep ];
else
    files.fe = ['fe_structure_' subj '_STC_run01_' track '_lmax' lmax '_connNUM' rep '.mat'];
    files.out     = ['fn_' subj '_aparc+aseg_' track '_lmax' lmax '_rep' rep '.mat'];
    files.csv     = ['fn_' subj '_aparc+aseg_' track '_lmax' lmax '_rep' rep ];
end
...
files.labels  = fullfile(files.path, 'subjects', subj, 'anat', 'aparc+aseg_inflated_3_vert.nii.gz');
...
save(files.output, 'pconn', 'omat', 'olab', 'files', 'glob', 'node', 'nets', '-append');
